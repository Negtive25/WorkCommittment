...
import org.com.code.im.service.PostCommentService;
...
public class PostCommentImpl implements PostCommentService {
    ...
    public PostCommentPageResponse getCommentsByPostIdWithCursor(PostCommentPageQuery commentPageQuery) {
        try {
            ...
            return new PostCommentPageResponse(commentList, new PostCommentPageQuery(curPageMaxId, curPageMinId, 0, commentPageQuery.getPostId(), null));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public PostCommentPageResponse getRepliesByParentIdWithCursor(PostCommentPageQuery commentPageQuery) {
        try {
            ...
            long curPageMinId = commentList.get(0).getAutoIncreasementId();
            long curPageMaxId = commentList.get(commentList.size()-1).getAutoIncreasementId();
            return new PostCommentPageResponse(commentList, new PostCommentPageQuery(curPageMaxId, curPageMinId, 0, commentPageQuery.getPostId(), commentPageQuery.getParentId()));
        } catch (Exception e) {
            e.printStackTrace();
