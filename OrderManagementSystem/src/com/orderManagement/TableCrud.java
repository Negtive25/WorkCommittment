...
public class TableCrud {
    ...
    public static void CreateTable(String tableSql) throws SQLException {
        try {
            //设置事务自动提交为false，开启事务
            CreateJdbcUtilObject.jdbcUtil.getConnection().setAutoCommit(false);

            //执行创建表的SQL语句
            CreateJdbcUtilObject.jdbcUtil.executeUpdate(tableSql);

            //提交事务
            CreateJdbcUtilObject.jdbcUtil.getConnection().commit();
        }catch (SQLException e) {
            //回滚事务
            CreateJdbcUtilObject.jdbcUtil.getConnection().rollback();
    ...
    public static void InsertProduct(String ProductID, String ProductName, String ProductPrice) throws SQLException {
        try {
            ...
            System.out.println("Transaction rolled back.");
            //释放资源
            CreateJdbcUtilObject.jdbcUtil.releaseSources();
            throw e;
        }
        CreateJdbcUtilObject.jdbcUtil.releaseSources();
    }
